...
def test_filter_salary():
    ...
    assert filter_salary({"from": 10, "to": 20}) == 15

    assert filter_salary({"from": 10, "to": None}) == 10

    assert filter_salary({"from": None, "to": 10}) == 10
...
def test_get_employers(employers):
    ...
    for company in employers:
        assert company["company"].get("name") == "Т-Банк"
